...
&nbsp;&nbsp;&nbsp;본 모델은 태아 심박 변동성(variability)과 심박 분포 통계 특성을 중심으로 건강 상태를 효과적으로 분류하며, 전체 정확도 약 90% 수준의 성능을 보였다. 다만 클래스 불균형 영향으로 인해 의심 및 병적 클래스의 recall이 낮아 위험군 일부를 정상으로 판단하는 한계가 나타난다. 향후에는 클래스 가중치 적용, 오버샘플링(SMOTE 등), Focal Loss, 혹은 임계값(threshold) 조정과 같은 불균형 대응 기법을 적용하여 위험군 재현율을 향상시키는 방향의 성능 개선이 요구된다.
