...
#### 3. 특성 중요도 평가
  ...
  랜덤 포레스트, XGBoost 등 트리 기반 모델을 사용해 각 특성의 예측 기여도를 계산합니다. 그 후 중요 특성을 도출하여 이후 모델 설계에 반영합니다.


### III.3 모델 구조 설계 (Model Architecture)
...
#### 3. 출력층
  ...
  3개의 클래스(정상, 의심, 병리적)에 대한 softmax 활성화를 통해 각 클래스에 대한 예측 확률을 제공합니다.


### III.4 학습 및 평가 방법 (Training & Evaluation)
...
#### 4. 성능 평가 지표
  ...
  정확도(전체 예측의 정밀성), F1-score(클래스 불균형을 고려한 조화평균), Confusion Matrix(클래수별 오분류 분석) 등 다양한 지표로 모델의 성능을 종합적으로 평가합니다.


### III.5 결과 해석 및 신뢰성 확보 (Model Interpretation & Reliability)
